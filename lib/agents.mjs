...
import { HOME, cyan, magenta, yellow, dim, die, shq, shq0, ssh, readLines, headEntries, extractText, normalizeIdPrefix } from "./util.mjs";
...
export const AGENTS = {
  claude: {
    ...
    resumeCmd: (s, targetPath) => `cd ${targetPath} && claude --resume ${s.id}`,
    ...
    async pull(r, host, localPath, force) {
      ...
      execFileSync("rsync", ["-a", `${host}:${r.file}`, dir + "/"], { stdio: "inherit" });
      const companion = r.file.slice(0, -6); // <uuid>/ sits next to <uuid>.jsonl
      if (ssh(host, `test -d ${shq(companion)} && echo yes || echo no`) === "yes")
        execFileSync("rsync", ["-a", `${host}:${companion}/`, path.join(dir, r.id) + "/"], { stdio: "inherit" });
      return dir;
    },
    async push(s, host, targetPath, force) {
      const dest = `~/.claude/projects/${ccSlug(targetPath)}`;
      if (!force && ssh(host, `test -f ${dest}/${s.id}.jsonl && echo yes || echo no`) === "yes")
        die("session already exists on remote - use --force");
      ssh(host, `mkdir -p ${dest} ${shq(targetPath)}`);
      execFileSync("rsync", ["-a", s.file, `${host}:${dest}/`], { stdio: "inherit" });
      const companion = s.file.slice(0, -6);
      if (fs.existsSync(companion))
        execFileSync("rsync", ["-a", companion + "/", `${host}:${dest}/${s.id}/`], { stdio: "inherit" });
    },
  },
  ...
  pi: {
    ...
    resumeCmd: (s, targetPath) => `cd ${targetPath} && pi --session ${s.id}`,
    ...
    async push(s, host, targetPath, force) {
      const dest = `~/.pi/agent/sessions/${piSlug(targetPath)}`;
      const fname = path.basename(s.file);
      if (!force && ssh(host, `test -f ${dest}/${shq(fname)} && echo yes || echo no`) === "yes")
        die("session already exists on remote - use --force");
      ssh(host, `mkdir -p ${dest} ${shq(targetPath)}`);
      execFileSync("rsync", ["-a", s.file, `${host}:${dest}/`], { stdio: "inherit" });
    },
  ...
  opencode: {
    ...
    resumeCmd: (s, targetPath) => `cd ${targetPath} && opencode -s ${s.id}`,
    ...
    async pull(r, host, localPath, force) {
      ...
      ssh(host, `opencode export ${shq(r.id)} > ${shq(tmpRemote)}`);
      execFileSync("rsync", ["-a", `${host}:${tmpRemote}`, tmpLocal], { stdio: "inherit" });
      ssh(host, `rm -f ${shq(tmpRemote)}`);
    ...
    async push(s, host, targetPath) {
      ...
      execFileSync("rsync", ["-a", tmpLocal, `${host}:${tmpRemote}`], { stdio: "inherit" });
      fs.unlinkSync(tmpLocal);
      ssh(host, `mkdir -p ${shq(targetPath)}`);
      const out = ssh(host, `cd ${shq(targetPath)} && opencode import ${tmpRemote}; rm -f ${tmpRemote}`);
      if (out) console.log(dim(out));
    },
  },
};
...
export function findSession(rawPrefix, { soft = false } = {}) {
  ...
  const matches = allSessions(null).filter((s) => s.id.startsWith(idPrefix));
  if (matches.length === 0) {
    if (soft) return null;

...
import {
  ...
  shortProject, snippet, absolutize, ensureStore, STORE_ROOT, loadContainerized, fitLine,
} from "./util.mjs";
...
// every listing row goes through here, so a narrow terminal truncates instead
// of wrapping and the columns stay lined up
const row = (line) => console.log(fitLine(line, process.stdout.columns));

// Column widths for a listing, given the terminal. The date, agent, id and
// context columns are fixed; what is left is split between the project and the
// opening message. Without this a narrow terminal spends everything on a padded
// project column and leaves one letter of preview, which is worse than useless -
// the preview is how you recognise the session you want.
function cols(fixed) {
  const total = process.stdout.columns || 120;
  const rest = Math.max(24, total - fixed);
  const project = Math.max(12, Math.min(26, Math.round(rest * 0.42)));
  return { project, preview: Math.max(12, rest - project - 2) };
}
...
export async function cmdList(args) {
  ...
  const w = cols(16 + 2 + 2 + 2 + 11 + 2 + 2 + 2 + 5 + 2);
  for (const s of sessions.slice(0, n)) {
    AGENTS[s.agent].enrich(s);
    const when = s.mtime.toISOString().slice(0, 16).replace("T", " ");
    row(
      `${dim(when)}  ${AGENTS[s.agent].tag()}  ${cyan(pad(s.id, 11))}  ${boxMark(s)}${green(pad(shortProject(s.cwd), w.project))}  ${ctxCol(ctxOf(s.agent, s))}  ${truncate(s.preview || dim("(untitled)"), w.preview)}`
    );
  }
  ...
  for (const name of activeAgents(agentFlagVal, store)) {
    ...
    if (A.search) {
      for (const h of A.search(term, n - hits, store)) {
        hits++; shown.push(h);
        row(`${dim(h.mtime.toISOString().slice(0, 10))}  ${A.tag()}  ${cyan(pad(h.id, 11))}  ${boxMark(h)}${green(pad(shortProject(h.cwd), 24))}  …${snippet(h.text, h.idx, term.length)}…`);
      }
      continue;
    }
    for (const s of A.sessions(store).sort((a, b) => b.mtime - a.mtime)) {
      ...
      if (found) {
        hits++; shown.push(s);
        A.enrich(s);
        row(`${dim(s.mtime.toISOString().slice(0, 10))}  ${A.tag()}  ${cyan(pad(s.id, 11))}  ${boxMark(s)}${green(pad(shortProject(s.cwd), 24))}  …${found}…`);
      }
    }
  }
  }
...
async function launchContainerized(s) {
  ...
  if (containerState(target) === null) {
    requireDocker();
    console.error(dim(`container ${name} is gone - rebuilding it from the image`));
...
function handOver(node, argv, cwd) {
  // cut rather than wrap: this line carries a full path and an elided prompt,
  // and wrapped across three rows it buries the command it is announcing
  console.error(fitLine(dim(`$ ${isRemote(node) ? `[${nodeLabel(node)}] ` : ""}${displayCmd(argv, cwd)}`), process.stderr.columns));
...
export async function cmdContainerize(args) {
  ...
  row(`session   ${AGENTS[s.agent].tag()} ${cyan(pad(s.id, 11))}  ${dim(truncate(s.preview ?? "", 50))}`);
  row(`project   ${s.cwd} ${dim("(mounted read-write at the same path)")}`);
  row(`container ${cyan(name)}\n`);
  // wrapped at 64 so it still reads on a narrow terminal
  for (const line of [
    "you are about to move this session into a dedicated",
    "container, to run its agent in bypass mode. the containerized",
    "agent gets the same settings and auth as your host agent.",
  ]) row(line);
...
// What the refresh actually achieved, in one trailing phrase. The seeded
// credential is only ever as good as the host's own, so an expired host token
// is worth saying out loud - re-seeding it would otherwise look like success.
function authNote(auth) {
  const c = auth.claude;
  if (!c) {
    if (auth.seeded.length) return dim(auth.seeded.map((s) => "~/" + s).join(", "));
    const agents = [...new Set(auth.missing.map((m) => m.agent))];
    return yellow(agents.length ? `no host credentials found for ${agents.join(", ")}` : "nothing to seed");
  }
  if (!c.expiresAt) return dim("seeded");
  const left = c.expiresAt - Date.now();
  return left <= 0
    ? yellow(`your host token expired ${fmtAge(-left / 1000)} ago - run any claude command here first`)
    : dim(`expires in ${fmtAge(left / 1000)}`);
}

// Credentials are copied into a container without their refresh token, so they
// expire and cannot heal themselves. Resuming tops them up, which covers the
// session you are about to sit in front of - and nothing else. A session left
// running unattended, or one whose agent hit the expiry mid-run, had no way back
// short of exiting and resuming it. This is that way.
//
// No argument means every containerized session, because "my containers have
// gone stale" is the usual reason to be here, and there is nothing to lose by
// topping up one that was already fine.
export async function cmdAuth(args) {
  const [idPrefix] = positionals(args);
  const recs = loadContainers();

  let names = Object.keys(recs);
  if (idPrefix) {
    const s = findSession(idPrefix);
    if (!s.store?.name) die(`session ${s.id.slice(0, 10)} is not containerized - it already uses your own login`);
    names = [s.store.name];
  }
  if (!names.length) { console.log(dim("no containerized sessions")); return; }
  requireDocker();

  for (const name of names) {
    const rec = recs[name];
    const target = rec.container ?? containerName(name);
    const label = pad(shortProject(rec.dir ?? ""), 30);
    // The container's own name stays out of it, as everywhere else: the project
    // is what you recognise a session by.
    const line = (status, note) => row(`${dim("▣")} ${label} ${status} ${note ?? ""}`.trimEnd());

    if (containerState(target) === null) {
      line(yellow("container gone"), dim("- rebuilt, and re-seeded, on next resume"));
      continue;
    }
    const why = await ensureRunning({ kind: "docker", name, target, cfg: rec });
    if (why) { line(yellow("unreachable"), dim(why)); continue; }
    try {
      const auth = reseedAuth({ target, agents: rec.agents ?? ["claude"], dir: rec.dir, store: rec.root });
      const copied = Boolean(auth.claude || auth.seeded.length);
      const partial = copied && auth.missing.length;
      line(copied ? (partial ? yellow("partially refreshed") : green("refreshed")) : yellow("not refreshed"), authNote(auth));
    } catch (e) {
      line(yellow("failed"), dim(e.message));
    }
  }
}
...
async function provision({ name, dir, agent, image, root, args, keepPorts = null, quiet = false }) {
  // Creating a container is ten to twenty seconds of silence otherwise, and a
  // first build is minutes. Say what is happening rather than looking hung.
  const step = (t) => console.log(dim(`  ${t}…`));
  if (!imageExists(image)) console.log(dim(`  building the ${image} image - first time only, this takes a few minutes…`));
  else step("checking the image is current");
  ...
  const c = createContainer({ name, dir, image, agents: [agent], keyPath: key, ports, store: root, onStep: step });

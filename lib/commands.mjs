...
function reportAuth(auth, agents, name) {
  const { seeded, missing, fromKeychain, expiresAt, withRefresh } = auth;
  if (seeded.length) console.log(`  auth      ${seeded.map((s) => "~/" + s).join(", ")}`);
  if (fromKeychain) {
    console.log(`  claude    ${green("seeded from the macOS Keychain")} ${dim("(Claude Code-credentials)")}`);
    if (withRefresh) {
      console.log(yellow(`            with its refresh token: the container can now rotate it, and`));
      console.log(yellow(`            a refresh here can invalidate your host login`));
    } else if (expiresAt) {
      const when = new Date(expiresAt);
      const rel = expiresAt > Date.now() ? `in ${fmtAge((expiresAt - Date.now()) / 1000)}` : "already";
      console.log(dim(`            access token only, so it cannot rotate your host login.`));
      console.log(dim(`            expires ${rel} (${when.toLocaleString()}) - then: casm container auth ${name}`));
    }
    return;
  }
  ...
  if (plan.kind === "env") {
    ...
    console.log(yellow(`  claude    no credentials to seed, and the Keychain read found nothing`));
    console.log(dim(`            fix with: claude setup-token, then export CLAUDE_CODE_OAUTH_TOKEN=<token>`));
    console.log(dim(`            or log in once inside: casm new --host ${name}`));
  }
}

...
export async function cmdContinue(args) {
  ...
  const all = allSessions(strFlag(args, "--agent", null));
  // bookmarked sessions are pinned to the top of the picker, newest first
  const marks = loadBookmarks();
  const sessions = [...all.filter((s) => s.id in marks), ...all.filter((s) => !(s.id in marks))].slice(0, n);
  ...
  sessions.forEach((s, i) => {
    ...
    console.log(
      `${cyan(pad(`${i + 1})`, 4))} ${dim(pad(fmtAge((Date.now() - s.mtime) / 1000) + " ago", 9))} ${AGENTS[s.agent].tag()}  ${green(pad(shortProject(s.cwd), 26))}  ${ctxCol(ctxOf(s.agent, s))}  ${truncate(s.preview || dim("(untitled)"), 48)}${mark}`
    );
  });

...
import {
  ...
  loadContainers, containerName, containerNames, containerState, ensureRunning, requireDocker,
} from "./nodes.mjs";
import {
  ...
  for (const name of activeAgents(agentFlagVal)) {
    ...
    if (A.search) {
      for (const h of A.search(term, n - hits)) {
        hits++;
        console.log(`${dim(h.mtime.toISOString().slice(0, 10))}  ${A.tag()}  ${cyan(pad(h.id, 11))}  ${green(pad(shortProject(h.cwd), 24))}  …${snippet(h.text, h.idx, term.length)}…`);
      }
      continue;
    }
...
export async function cmdContinue(args) {
  ...
  const nodes = hostArg ? [await targetNode(args)]
    : args.includes("--local") ? [LOCAL]
    : [LOCAL, ...containerNames().map(resolveNode)];
  ...
  sessions.forEach((s, i) => {
    ...
    console.log(
      `${cyan(pad(`${i + 1})`, 4))} ${dim(pad(fmtAge((Date.now() - s.mtime) / 1000) + " ago", 9))} ${AGENTS[s.agent].tag()}${where}  ${green(pad(shortProject(s.cwd), 26))}  ${ctxCol(isRemote(s.node) ? s.ctx ?? null : ctxOf(s.agent, s))}  ${truncate(s.preview || dim("(untitled)"), multi ? 34 : 48)}${mark}`
    );
  });
...
const CONTAINER_USAGE = `usage: casm container list
       casm container create <name> --dir <path> [--agent claude,opencode,pi] [--image casm/agents]
                                                 [--ports 5] [--no-ports] [--no-keychain] [--no-sudo]
       casm container start [<name>...]          (rarely needed - see below)
       ...
       casm container build [--image casm/agents] [--base node:22-bookworm-slim]
                                                 (full rebuild, no cache; create builds cached)
...
a stopped container is started automatically by anything that needs it -
container list, list, active, search, continue, resume, new, push and pull - so
a reboot costs you nothing. its sessions survive being stopped either way.`;

const validContainerName = (n) => /^[A-Za-z0-9][A-Za-z0-9._-]*$/.test(n);

// A container here, by casm name, without going through resolveNode: the
// container subcommands address casm's own containers directly, including ones
// that carry the label but are no longer in config.json, which resolveNode
// would take for an ssh target.
const dockerNode = (name) => ({
  kind: "docker", name, target: loadContainers()[name]?.container ?? containerName(name),
});
...
export async function cmdContainer(args) {
  ...
  if (sub === "start") return containerStart(rest);
...
async function containerList() {
  ...
  let rows = new Map(containerRows().map((r) => [r.name, r]));
  const names = [...new Set([...Object.keys(registered), ...rows.keys()])].sort();
  if (!names.length) {
  ...
  // A reboot leaves every container stopped, so listing them is exactly the
  // moment to bring them back rather than to report a wall of `exited`. Started
  // concurrently and re-read afterwards, so the states printed are the ones that
  // hold now and not the ones from before the starts.
  const down = names.filter((n) => rows.get(n) && rows.get(n).state !== "running");
  if (down.length) {
    await Promise.all(down.map((n) => ensureRunning(dockerNode(n))));
    rows = new Map(containerRows().map((r) => [r.name, r]));
  }
  ...
  for (const name of names) {
    ...
    const ports = cfg.ports ? `${cfg.ports[0]}-${cfg.ports[1]}` : "";
    console.log(`${green(pad(name, 18))} ${state} ${dim(pad(ports, 12))} ${dim(pad(cfg.image ?? row?.image ?? "?", 18))} ${cfg.dir ?? row?.dir ?? ""}`);
  }
  console.log(dim(`\n${CONTAINER_PATH_HINT}`));
}
...
async function containerCreate(rest, args) {
  ...
  // Always build, so a container is never created from an image that predates
  // the casm you are running - exactly how a stale image survived an upgrade and
  // produced containers without sudo. Cached, so an unchanged Dockerfile costs
  // well under a second and a changed one rebuilds only what moved. Refreshing
  // the agent CLIs themselves is `casm container build`, which skips the cache.
  buildImage(image, strFlag(args, "--base", null), { quiet: imageExists(image) });
  ...
  // A block of published ports, so the agent can serve something you can open.
  // Skipped entirely with --no-ports; sized with --ports N.
  let ports = null;
  if (!args.includes("--no-ports")) {
    const size = Math.max(1, intFlag(args, "--ports", PORT_BLOCK));
    ports = await allocatePorts(Object.values(loadContainers()).map((c) => c.ports).filter(Boolean), size);
  ...
  const cfg = loadConfigForWrite();
  cfg.containers = { ...(loadContainers()), [name]: { container: c.target, dir, image, agents, ...(ports ? { ports } : {}) } };
  saveConfig(cfg);
  ...
  if (ports) console.log(`  ports     ${ports[0]}-${ports[1]} ${dim("published as-is; CASM_PORTS and PORT are set inside")}`);
...
// Explicit, for a script that wants a container up before it does anything with
// it. Interactively you rarely need it: list/continue/resume/new/push all start
// a stopped container on their own.
async function containerStart(rest) {
  const names = rest.length ? rest : containerRows().map((r) => r.name);
  if (!names.length) die("no containers here");
  for (const name of names) {
    if (containerState(dockerNode(name).target) === "running") { console.log(`${green(pad(name, 18))} ${dim("already running")}`); continue; }
    const why = await ensureRunning(dockerNode(name));
    console.log(why ? `${yellow(pad(name, 18))} ${why}` : `${green(pad(name, 18))} started`);
  }
}
...
// The explicit command always rebuilds from scratch. Layer caching pins the
// agent CLIs to whenever the layer was built, so a cached rebuild would leave
// them exactly as they were, which is not what someone asking for a rebuild
// wants. `create` is the cached path.
function containerBuild(args) {
  buildImage(strFlag(args, "--image", DEFAULT_IMAGE), strFlag(args, "--base", null), { noCache: true });
}

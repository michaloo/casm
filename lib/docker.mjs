...
export function buildRunArgs({ name, image, dir, uid, gid, limits = {}, ports = null }) {
  ...
  for (const [rel] of RO_MOUNTS) {
    ...
    if (fs.existsSync(host)) args.push("-v", `${host}:${path.posix.join(CHOME, rel)}:ro`);
  }
  if (ports) {
...
export function createContainer({ name, dir, image = DEFAULT_IMAGE, agents = ["claude"], keyPath, keychain = true, withRefresh = false, sudo = true, ports = null }) {
  ...
  const r = dockerTry(buildRunArgs({ name, image, dir, uid, gid, ports }));
  ...
  const auth = seedAuth(target, agents, { keychain, withRefresh, dir });
...
function seedAuth(target, agents, { keychain = true, withRefresh = false, dir = null } = {}) {
  const seeded = [];
  const missing = [];
  for (const a of agents) {
    for (const rel of AUTH_SEED[a] ?? []) {
      ...
      docker(["exec", target, "mkdir", "-p", path.posix.join(CHOME, path.dirname(rel))]);
      docker(["cp", host, `${target}:${path.posix.join(CHOME, rel)}`]);
      seeded.push(rel);
    }
  }
  ...
  if (keychain && agents.includes("claude")) {
    ...
    if (claude) {
      seedClaudeState(target, dir);
      writeSecret(target, `${CHOME}/.claude/.credentials.json`, claude.json);
    } else {
...
export function reseedAuth({ target, agents, keychain = true, withRefresh = false, dir = null }) {
  const uid = process.getuid?.() ?? 1000;
  const gid = process.getgid?.() ?? 1000;
  const auth = seedAuth(target, agents, { keychain, withRefresh, dir });
...
function seedClaudeState(target, dir) {
  ...
  writeSecret(target, `${CHOME}/${CLAUDE_STATE}`, JSON.stringify(trimmed));
  return true;
}

...
//   ~/.codex/config.toml            - codex writes to it (model choice, migration
//                                      prompts), so a read-only mount makes every
//                                      such write fail loudly. Copied in instead.
...
// [path relative to home, follows the store]. The second field matters as much
// as the first: a store redirects CLAUDE_CONFIG_DIR and CODEX_HOME, so those
// agents read their config from inside it and a mount at $HOME is simply never
// looked at. opencode's config dir is XDG_CONFIG_HOME-based and pi's config dir
// is untouched, so both stay where they are.
...
export const CONTAINER_BRIEF = [
  ...
  "- Anything you install (apt packages, global npm packages, language runtimes, services) is lost on a reset.",
  "- Record installs and OS-level setup in the script at $CASM_SETUP as you go, and keep it idempotent - casm re-runs it after a reset to rebuild this environment.",
  "- $CASM_SETUP is the only place for that. Do not add setup instructions, Dockerfiles or environment notes to the project's own files unless they belong there for the project's own sake.",
...
export function createContainer({ name, dir, image = DEFAULT_IMAGE, agents = ["claude"], keyPath, keychain = true, withRefresh = false, sudo = true, ports = null, store = null }) {
...
function claimHome(target, uid, gid, sudo = true) {
  ...
  if (sudo) {
    docker(["exec", "-u", "0", target, "sh", "-c",
      `printf '#%s\\n' "casm: passwordless sudo, see casm container create --no-sudo" > /etc/sudoers.d/casm; ` +
...
function seedConfig(target, store = null) {
  for (const [rel, inStore] of SEED_CONFIG) {
    ...
    docker(["exec", target, "mkdir", "-p", path.posix.dirname(dest)]);
    try { docker(["cp", host, `${target}:${dest}`]); } catch {}
  }
}
...
function seedClaudeState(target, dir, store = null) {
  ...
  // Same reasoning, one step further. The container exists to run an agent with
  // prompts off; claude still shows its bypass-mode warning and waits for you to
  // accept it, which is a dialog whose answer casm already decided when it wrote
  // the managed policy. Without this every containerized session opens on a
  // modal, and an unattended one simply sits there forever.
  trimmed.bypassPermissionsModeAccepted = true;
...
// expires and then stops, which is visible and fixed by `casm container auth`.

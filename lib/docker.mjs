...
const AUTH_SEED = {
  claude: [".claude/.credentials.json"], // absent on macOS - see checkClaudeAuth
...
export function createContainer({ name, dir, image = DEFAULT_IMAGE, agents = ["claude"], keyPath, keychain = true, withRefresh = false }) {
  ...
  // The permissive policy has to be root-owned so the agent, which runs as the
  // host uid, cannot edit its own policy.
  const tmp = path.join(os.tmpdir(), `casm-managed-${name}.json`);
  fs.writeFileSync(tmp, MANAGED_SETTINGS + "\n");
  docker(["exec", "-u", "0", target, "mkdir", "-p", "/etc/claude-code"]);
  docker(["cp", tmp, `${target}:/etc/claude-code/managed-settings.json`]);
  docker(["exec", "-u", "0", target, "chown", "root:root", "/etc/claude-code/managed-settings.json"]);
  docker(["exec", "-u", "0", target, "chmod", "0644", "/etc/claude-code/managed-settings.json"]);
  fs.unlinkSync(tmp);

// shared helpers: terminal colors, config, ssh, file readers
...
export function ssh(host, cmd) {
  return execFileSync("ssh", [host, cmd], { encoding: "utf8", maxBuffer: 32 * 1024 * 1024 }).trim();
}
// run casm on another node and capture its output. `bash -lc` so the login
// PATH (where npm puts global bins) is available; never prompts, never hangs.
export function casmRemote(host, args, connectTimeout = 10) {
  const inner = ["casm", ...args].map(shq).join(" ");
  return new Promise((res) => {
    execFile("ssh", ["-o", "BatchMode=yes", "-o", `ConnectTimeout=${connectTimeout}`, host, `bash -lc ${shq(inner)}`],
      { encoding: "utf8", maxBuffer: 32 * 1024 * 1024 }, (err, stdout, stderr) =>
        res({ ok: !err, out: (stdout ?? "").trimEnd(), err: (stderr ?? "").trimEnd() }));
  });
}
// non-throwing probe: never prompts (BatchMode) and never hangs on a dead host
export function sshTry(host, cmd, connectTimeout = 6) {
  return new Promise((res) => {
    execFile("ssh", ["-o", "BatchMode=yes", "-o", `ConnectTimeout=${connectTimeout}`, host, cmd],
      { encoding: "utf8", maxBuffer: 1024 * 1024 }, (err, stdout, stderr) =>
        res({ ok: !err, out: (stdout ?? "").trim(), err: (stderr ?? "").trim().split("\n").pop() ?? "" }));
  });
}

...
import { plain, pad, truncate, snippet, shortProject, fitLine, strFlag, intFlag } from "./util.mjs";
...
test("flag readers accept space-separated and equals values", () => {
  assert.equal(strFlag(["--agent", "codex"], "--agent", null), "codex");
  assert.equal(strFlag(["--agent=codex"], "--agent", null), "codex");
  assert.equal(intFlag(["-n=12"], "-n", 20), 12);
});
